...
function Cancelled({shipmentStatus}) {
  ...
  return (
    <div className='w-full flex flex-col p-4 gap-[20px]'>
      <div className={`w-full flex text-white text-xs h-[5px] relative ${lang === 'ar' ? 'flex-row-reverse' : ''}`}>
        <p className='flex-1 bg-red-600 relative'>
          <span className={`absolute w-[15px] h-[15px] text-center bg-red-600 rounded-full top-[50%] -translate-y-1/2 ${lang === 'ar' ? 'right-[-1px] md:right-[0px]' : "left-[-1px] md:left-[0px]"}`}>&#10004;</span>
        </p>
        <p className='flex-1 bg-red-600'>
          <span className={`absolute w-[15px] h-[15px] text-center bg-red-600 rounded-full top-[50%] -translate-y-1/2 ${lang === 'ar' ? 'right-[33%]' : "left-[33%]"}`}>&#10004;</span>
        </p>
        <p className='bg-red-600'>
          <span className={`absolute w-[30px] h-[30px] flex justify-center items-center bg-red-600 rounded-full top-[50%] -translate-y-1/2 ${lang === 'ar' ? 'right-[66%]' : "left-[66%]"}`}>{lang === 'ar' ? <img src={reversedVan} alt='van' className='text-white h-[22px]'/> : <img src={van} alt='van' className='text-white h-[22px]'/>}</span>
        </p>
      ...
      <div className={`flex justify-between text-xs ${lang === 'ar' ? 'flex-row-reverse' : ''}`}>
        ...
        <p className='flex px-2 flex-col justify-center items-center gap-[5px]'>
          <span className='text-center'>{t('OUT_FOR_DELIVERY')}</span>
          <span className='text-red-600 text-center'>{t(shipmentStatus)}</span>
        </p>
